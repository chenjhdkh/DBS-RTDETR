...
__all__ = ['Ghost_HGBlock', 'Rep_HGBlock', 'DWRC3', 'C3_DWR', 'C2f_DWR', 'BasicBlock_Ortho', 'BottleNeck_Ortho',
           ...
           'BasicBlock_Attention', 'BottleNeck_Attention','BottleNeck_SPABlock', 'HGBlock_Attention', 'C3_Attention', 'C2f_Attention',
...
class BasicBlock_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
from butterfly import Fusionbft
...
class BottleNeck_Attention(nn.Module):
    ...
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d'):
        ...
        self.conv2 = nn.Conv2d(width, width, kernel_size=3,
                                stride=stride2, padding=1, groups=width, bias=False)
        self.bn2 = nn.BatchNorm2d(width)
        # self.branch2b = WTConv2d(ch_out, ch_out, stride=stride)
        self.branch2c = Fusionbft(width, ch_out * self.expansion)
        ...
        self.attention = MoCAttention(InChannels=ch_out * self.expansion, HidChannels=ch_out, SqueezeFactor=4)
    def forward(self, x):
        out = self.branch2a(x)
        # out = self.branch2b(out)
        ...
        return out
class BottleNeck_SPABlock(nn.Module):
    expansion = 4
    ...
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d'):
        ...
        self.attention = SPANet(in_channels=ch_out * self.expansion, adaptive=True).cuda()
        # self.mdcr = MDCR(ch_out * self.expansion, ch_out * self.expansion)
...
class Bottleneck_Attention1(nn.Module):
    """Standard bottleneck."""
